...
int main(int argc, char **argv) {
  ...
  CLOG(WARNING, "text") << initial_timestamp_micro_;

  // Gyroscope data
  std::vector<Eigen::MatrixXd> gyro_data_;
  std::vector<Eigen::Vector3d> const_gyro_bias;
  for (size_t i = 0; i < 4; ++i) {
    std::string gyro_path = odo_dir.string() + "/" + lnames[i] + "_imu.csv";
  ...
  for (const auto &dir_entry : fs::directory_iterator{odo_dir / lnames[3]})
